...
## TL;DR
- 數字排列:用 `rotate` 旋轉 12 個數字放到圓周上。
- 指針轉動:`setInterval` 每秒更新時分秒針角度。
- 時間計算:時針 30°/小時、分針 6°/分鐘、秒針 6°/秒。

## 前置知識
- CSS `position: absolute`、`transform-origin`、`rotate`
- JavaScript `Date` 物件與時間取得
- `setInterval` 定時執行函數
...
- [CSS rotate 與 transform-origin](https://developer.mozilla.org/en-US/docs/Web/CSS/rotate)
- [setInterval vs requestAnimationFrame](https://javascript.info/settimeout-setinterval)

## FAQ
- Q: 為何時針角度要加 `m / 2`?
  - A: 時針不只跟小時有關,每分鐘也會移動 0.5° (30° / 60分),所以要加上分鐘的影響。
- Q: 如何讓秒針平滑轉動?
  - A: 用 `requestAnimationFrame` 取代 `setInterval`,並用毫秒計算角度: `s + ms / 1000`。
- Q: 數字為何要反轉回來?
  - A: 外層 div 旋轉後,內層數字也跟著旋轉了,需要在內層再反轉回正: `<span style="rotate: -${idx * 30}deg">${num}</span>`。
