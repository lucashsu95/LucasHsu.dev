...
先寫出 HTML 骨架。畫面中需要一個顯示數字的地方和三個按鈕。
...
說明：
- `<span id="value">` 負責顯示數字，`id` 讓 JavaScript 能快速找到它
- 三個按鈕各自對應一個 class：`.decrease`（減少）、`.reset`（重置）、`.increase`（增加）
- 最後引入 `app.js`，串接 JavaScript 邏輯
...
說明：
- 使用 flexbox 讓畫面置中
- 按鈕各有不同顏色：紅色（減少）、灰色（重置）、綠色（增加）
- `.positive`、`.negative`、`.zero` 控制數字顏色，由 JavaScript 動態切換
- 按鈕有 hover 浮起和 active 按壓效果

### Step 3: JavaScript 邏輯

JavaScript 是計數器的大腦。我們分成 4 個小步驟來寫。
...
以下是完整的 HTML、CSS 和 JavaScript 程式碼，可直接複製使用。
