...
<p class="lead">先把窄螢幕寫死，再用 `min-width` media query 逐步加強。breakpoint 由內容何時失衡決定，不是裝置品牌。</p>
...
<div v-click class="callout">基礎規則服務窄空間，再以 min-width 加強。breakpoint 看內容失衡，不看裝置品牌。</div>

---

# Media query vs Container query

<p class="lead">Media query 回應 viewport 與裝置能力；container query 讓元件看自己的可用空間。同一卡片放 sidebar 或 main 都能自治。</p>
...
<p class="lead">要回應的是整頁，還是元件自己的空間？前者用 media query，後者用 container query；只偵測 CSS 支援才用 `@supports`。</p>
...
<p class="lead">動效、配色、對比度 — 這些 media feature 讓介面更包容，不只是靠 breakpoint。</p>

<div class="card-grid">
  <div v-click class="lab-card"><b>Reduced motion</b><code>@media (prefers-reduced-motion: reduce)</code><p>減少非必要位移與視差，別粗暴關掉所有回饋。</p></div>
  ...
  <div v-click class="lab-card"><b>Contrast</b><code>@media (prefers-contrast: more)</code><p>加強邊界、文字與狀態差異；先確認目標支援。</p></div>
</div>
...
# Responsive images：別讓手機下載 4K

<p class="lab-note">兩層：`srcset` + `sizes` 選解析度，`picture` 處理裁切或格式不同的 art direction。</p>
...
<div class="stack">
  <div v-click><b>`srcset` + `sizes`</b><span>解析度選擇：給瀏覽器候選檔與預期渲染寬度。</span></div>
  <div v-click><b>`picture`</b><span>Art direction / 格式選擇：來源內容或裁切真的不同時用。</span></div>
  <div v-click><b>width + height</b><span>提供 intrinsic ratio，直接減少 layout shift。</span></div>
</div>

---

# Shiki Magic Move：從 viewport breakpoint 到元件自治

<p class="lead">把版面規則從「全站 viewport breakpoint」搬到「元件容器 query」。同一 ProductCard 不用為每個版面寫一組 media query。</p>
...
<p class="lead">拖曳右側把手改變卡片容器寬度，看窄於 400px 時自動切單欄、寬於閾值時維持雙欄；左側 Monaco 可調 grid、gap、字級。</p>
...
<p class="lab-note">viewport media query 管整頁；元件在 sidebar 與 main 寬度不同時，讓它回應自己的容器，別猜使用者裝置。</p>

<div class="quiz">
  <p>同一張 ProductCard 在 320px sidebar 與 700px main 中各自變版，首選？</p>
...
<p class="lead">好的 RWD 同時處理空間、能力、偏好與內容。用 `@media` 與 `@container` 選對偵測對象，比維護裝置清單更可靠。</p>
