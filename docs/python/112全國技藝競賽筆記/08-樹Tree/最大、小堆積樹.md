...
def isMinHeap():
    ...
    for i in range(1, n // 2 + 1):
        if tree[i] > tree[i * 2]:              # 左子一定存在
            return False
        if i * 2 + 1 <= n and tree[i] > tree[i * 2 + 1]:  # 右子先確認
            return False
    return True
...
def isMaxHeap():
    ...
    for i in range(1, n // 2 + 1):
        if tree[i] < tree[i * 2]:
            return False
        if i * 2 + 1 <= n and tree[i] < tree[i * 2 + 1]:
            return False
    return True

for _ in range(int(input())):
    tree = [0] + list(map(int, input().split(',')))
    print('T' if isMinHeap() or isMaxHeap() else 'F')
'''
def check(op):
    n = len(tree) - 1
    for i in range(1, n // 2 + 1):
        if op(tree[i], tree[i*2]): return False
        if i*2+1 <= n and op(tree[i], tree[i*2+1]): return False
    return True

for _ in range(int(input())):
    tree = [0] + list(map(int, input().split(',')))
    print('T' if check(lambda a,b: a>b) or check(lambda a,b: a<b) else 'F')
'''
```
