...
head:
  ...
  - - meta
    - name: og:title
      content: Python新手上路 Day2 - 迴圈和字串操作
  - - meta
    - name: og:description
...
# python-新手上路-Day2｜for｜while｜字串切片
...
Python使用從0開始的索引來識別序列中的元素。換句話說，第一個元素的索引是0，第二個元素的索引是1，依此類推。
...
在Python中，迴圈是一種控制結構，用於重複執行特定的程式碼塊，直到某個條件滿足或循環次數完成為止。Python提供了兩種主要的迴圈類型：`for` 迴圈和 `while` 迴圈。
